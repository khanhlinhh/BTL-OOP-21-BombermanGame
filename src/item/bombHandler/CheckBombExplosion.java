...
public class CheckBombExplosion {
    ...
    public void BombActivated(Bomb other, Bomb bomb) {
        ...
        if ((bottom >= topBomb && bottom <= bottomBomb) || (top <= bottomBomb && top >= topBomb)) {
            ...
            else if (bomb.worldX <= left && left <= bomb.worldX + GamePanel.tileSize) other.countExplode = 0;
        } else if ((left >= leftBomb && left <= rightBomb) || (right <= rightBomb && right >= leftBomb)) {
            if (bomb.worldY <= bottom && bottom <= bomb.worldY + GamePanel.tileSize) other.countExplode = 0;
            else if (bomb.worldY <= top && top <= bomb.worldY + GamePanel.tileSize) other.countExplode = 0;
        }
    }
